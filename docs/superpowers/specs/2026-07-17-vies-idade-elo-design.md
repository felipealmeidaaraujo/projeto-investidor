...
7. **O ajuste extrapola para Challenger, onde NÃO foi validado.** ⚠️ Apurado na review final (2026-07-17). O coeficiente 0,026 tem IC acima de zero **só no nível tour** (o teste fora da amostra filtrou `src === 'tour'`, N=6.618). Mas a grade do dia vem do Flashscore, que **rotula Challenger e ITF como "ATP"** ([flashscore.js:13](../../../pipeline/flashscore.js) descarta o nível — ver [jogos-do-dia-fonte-espn]), então o ajuste roda também nesses jogos. Medido: **4 dos 7 jogos ajustados num dia típico eram Challenger** (Granby, Pozoblanco). É extrapolação — a **mesma categoria de risco que fez esta spec rejeitar a WTA**, passando na aplicação sem sinalização. A probabilidade servida continua correta (o Elo é o mesmo), mas a correção de idade não tem suporte empírico ali. **O comentário do `AGE_COEF` avisa disso.** A correção certa é gatear por nível do torneio — que depende de o `flashscore.js` parar de descartar o nível (outra spec).
